...
```{r}
san.Kenya.2020.22 <- sanitation |> 
  filter(name == "Kenya", year %in% c(2000,2020), varname_short != "san_sm")
```
...
```{r}
san.Kenya.2020.22 |> 
  ggplot(aes(x = residence, y = percent, fill = varname_long)) +
  geom_col() +
...
2.  What would you want to change? order the categories so that the order starts with no san facilities and improves from there
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? some countries don't have data on safely managed sanitation
